...
def acqf_optimizer(
    ...
    def cons_J(embed_test: np.ndarray):
        ...
        def up_projection(zz):
            ...
            mean, stddev = mvn.mean.view(-1), mvn.stddev.view(-1)
            return mean, stddev
        dmean, dstddev = torch.autograd.functional.jacobian(up_projection, xx)
        return torch.cat((dmean - dstddev, dmean + dstddev), dim=0).cpu().numpy()
    ...
    nonlinear_constraint = NonlinearConstraint(
        ...
        # jac=cons_J,
        # hess=lambda t : np.zeros((high_dim, emb_dim, emb_dim)),
        keep_feasible=True
    )
...
def mkdr_bo_scale(
    ...
    for iter in range(n_iterations):
        ...
        batchgp = SingleTaskGP(
            train_X=x_embedded, train_Y=X,
            train_Yvar=torch.full_like(X, 1e-6).to(X),
            covar_module=ScaleKernel(
                # base_kernel=RBFKernel(
                #     ard_num_dims=x_embedded.shape[-1],
                #     batch_shape=torch.Size([batch_size]),
                # ),
                base_kernel=LinearKernel(
                    num_dimensions=x_embedded.shape[-1],
                    batch_shape=torch.Size([batch_size])
                ),
                batch_shape=torch.Size([batch_size]),
            )
        ...
        logger.info(f"Runned trial {iter}. batchGP: {(t3-t2)/60:.2f}. num_gen: {num_gen}. Max acqf: {(t4-t3)/60:.2f}min")
