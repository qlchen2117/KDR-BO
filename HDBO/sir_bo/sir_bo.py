...
class SIRBO:
    ...
    def update_model(self, f_t, final_x_at_minY):
        ...
        self.emb[self.num] = final_x_at_min
...
def opt(
    ...
    for trail in range(num_iter):
        final_x_at_min = maximize_acq(model, dopt, 'ei')

        f_t = objective((final_x_at_min + 1) / 2)
        final_x_at_min = final_x_at_min[np.newaxis, :]
        model.update_model(f_t, final_x_at_min)
        wallclocks[trail] = time.monotonic() - startT
    return model, wallclocks


def sir_bo(obj_fct, total_iter, dim, high_dim, init_size):
