...
pick_data, DIM = 'mopta', 124

EM_DIM = 10
# Read results
res_p = Path("results/mopta/final")
...
## HDBO
store_data = np.load(res_p.joinpath(f"{pick_data}-D{DIM}-turboD.npy"))
ax.plot(np.minimum.accumulate(store_data, axis=1).mean(0), label="TuRBO-D")
store_data = np.load(res_p.joinpath(f"{pick_data}-D{DIM}-turbo.npy"))
ax.plot(np.minimum.accumulate(store_data, axis=1).mean(0), label="TuRBO")
store_data = np.load(res_p.joinpath(f"{pick_data}-D{DIM}-d{EM_DIM}-rembo.npy"))
ax.plot(np.minimum.accumulate(store_data, axis=1).mean(0), label="REMBO")
store_data = np.load(res_p.joinpath(f"{pick_data}-D{DIM}-d{EM_DIM}-alebo.npy"))
ax.plot(np.minimum.accumulate(store_data, axis=1).mean(0), label="ALEBO")
store_data = np.load(res_p.joinpath(f"{pick_data}-D{DIM}-d{EM_DIM}-ksir_bo.npy"))
ax.plot(np.minimum.accumulate(store_data, axis=1).mean(0), label="KSIR-BO")
store_data = np.load(res_p.joinpath(f"{pick_data}-D{DIM}-d{EM_DIM}-sir_bo.npy"))
ax.plot(np.minimum.accumulate(store_data, axis=1).mean(0), label="SIR-BO")
store_data = np.load(res_p.joinpath(f"{pick_data}-D{DIM}-sobol.npy"))
ax.plot(np.minimum.accumulate(store_data, axis=1).mean(0), label="Sobol")
...
ax.grid(True)
ax.set_title(f"Mopta (D = {DIM})", fontsize=18)
