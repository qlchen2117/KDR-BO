...
def testNASBench(N_EPOCH=10, N_INIT=15, N_ITERACTIONS=100, BATCH_SIZE=1):
    DIM = 36
    EM_DIM = 10
    ...
    #     r = NASBenchRunner(max_eval=TOTAL_TRIALS)
    #     _, Y = rembo(eval_objective, D=DIM, d=EM_DIM, n_init=N_INIT, total_trials=TOTAL_TRIALS)
    #     Y_np = -1 * Y
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p.joinpath(f"nas-D{DIM}-d{EM_DIM}-rembo.npy"), store_data)
    ...
    # np.save(res_p.joinpath(f"nas-D{DIM}-turbo.npy"), store_data)

    # from HDBO.turboD_highD import turboD
    from HDBO.turboD import turboD
    for i in range(N_EPOCH):
        r = NASBenchRunner(max_eval=2*TOTAL_TRIALS)
        _, Y = turboD(
    ...
    np.save(res_p.joinpath(f"nas-D{DIM}-turboD.npy"), store_data)
    ...
    #     Y_np = -1 * Y
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p.joinpath(f"nas-D{DIM}-d{EM_DIM}-alebo.npy"), store_data)
    ...
    #     Y_np = -1 * Y.cpu().numpy()
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p.joinpath(f"nas-D{DIM}-d{EM_DIM}-kdr_bo.npy"), store_data)
    ...
    #     Y_np = -1 * Y.cpu().numpy()
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p.joinpath(f"nas-D{DIM}-d{EM_DIM}-mkdr_bo.npy"), store_data)
    ...
    #     Y_np = -1 * Y.cpu().numpy()
    #     store_data[i] = Y_np.ravel()
    # np.save(res_p.joinpath(f"nas-D{DIM}-sobol.npy"), store_data)
    ...
    # np.save(res_p.joinpath(f"nas-D{DIM}-d{EM_DIM}-sir_bo.npy"), store_data)
    ...
    #     store_data[i] = Y.ravel()
    # np.save(res_p.joinpath(f"nas-D{DIM}-d{EM_DIM}-ksir_bo.npy"), store_data)
    ...
    # np.save(res_p.joinpath(f"nas-D{DIM}-add_bo.npy"), store_data)
